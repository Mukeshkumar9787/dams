              ...
              <AdminMobileList
                ...
                renderCard={(item, index) => (
                  <div className="admin-mobile-card">
                    <div className="mb-4">
                      <p className="text-xs font-semibold uppercase tracking-[0.12em] text-dark-5">User #{(pagination.pageNumber - 1) * pagination.pageSize + index + 1}</p>
                      <h3 className="text-base font-semibold text-dark">{item.name}</h3>
                      <p className="break-all text-sm text-dark-4">{item.email}</p>
                    </div>
                    <div className="admin-mobile-meta">
                      <span className="admin-mobile-label">Mobile</span>
                      <span className="admin-mobile-value">{item.mobile || "-"}</span>
                    </div>
                    <div className="admin-mobile-meta">
                      <span className="admin-mobile-label">Role</span>
                      <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${item.role === ROLE_TYPES.ADMIN ? "bg-blue/15 text-blue-dark" : "bg-orange/15 text-orange-dark"}`}>
                        {item.role}
                      </span>
                    </div>
                    <div className="mt-4">
                      <Button className="w-full rounded-xl bg-blue px-3 text-white" onClick={() => setEditUser(item)}>Change Role</Button>
                    </div>
                  </div>
                )}
              />
