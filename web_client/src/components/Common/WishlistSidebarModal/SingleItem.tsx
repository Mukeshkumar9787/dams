      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex w-full items-start gap-4">
          <Link
            ...
            className="flex h-24 w-24 flex-shrink-0 items-center justify-center rounded-[20px] bg-slate-100"
          >
            <img src={item.img} alt="product" width={100} height={100} className="max-h-full rounded-[18px] object-cover" />
          </Link>
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-2 text-sm text-dark-4">
              Price: {getCurrencyDetails().currencySymbol} {item.price}
            </p>
            <Link
              href={`/shop-details/${item.slug}`}
              onClick={() => closeWishlistModal()}
              className="mt-4 inline-flex rounded-2xl border border-blue bg-blue px-4 py-2 text-sm font-medium text-white transition hover:bg-blue-dark"
            >
              View Product
            </Link>
          </div>
        ...
        <Popconfirm
          ...
          cancelText="Cancel"
          okType="primary"
          okButtonProps={{ className: "bg-blue text-white" }}
          onConfirm={() => dispatch(removeItemFromWishlist(item.id))}
        >
          <button
            type="button"
            aria-label="button for remove product from wishlist"
            className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full border border-slate-200 bg-slate-50 text-dark transition hover:border-red-200 hover:bg-red-50 hover:text-red"
          >
          <svg
